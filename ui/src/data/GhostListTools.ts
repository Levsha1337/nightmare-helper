...
export default class GhostListTools {
    public static calculatePossibleGhosts(evidences: Evidence[], maxCount: number) {
        if (evidences.length === 0) {
            return GhostList;
        }

        return GhostList.filter((ghost) => ghost.resolve(ghost, evidences, maxCount));
    }

    public static calculatePossibleEvidences(evidences: Evidence[], maxCount: number): Evidence[] {
